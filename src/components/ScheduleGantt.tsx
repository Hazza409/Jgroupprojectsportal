...
// month/quarter time axis. Client component so the timeline can zoom; the chart
// scrolls horizontally inside the card when wider than the screen.
...
export function ScheduleGantt({ items }: { items: Item[] }) {
  ...
  const min = startOfMonth(new Date(Math.min(...dated.map((i) => new Date(i.startDate!).getTime()))));
  const maxEnd = new Date(Math.max(...dated.map((i) => new Date(i.endDate!).getTime())));
  ...
  const timelineMin = Math.round(820 * zoom);
  const zoomBy = (f: number) => setZoom((z) => Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, Math.round(z * f * 100) / 100)));

  return (
    <div className="card p-0">
      {/* Zoom toolbar */}
      <div className="flex items-center justify-end gap-1 border-b border-stone-200 px-3 py-2 text-xs text-stone-500">
        <span className="mr-2 hidden sm:inline">Scroll sideways to pan</span>
        <button type="button" className="btn-ghost !px-2.5 !py-1" onClick={() => zoomBy(1 / 1.25)} disabled={zoom <= ZOOM_MIN} aria-label="Zoom out">−</button>
        <span className="w-12 text-center tabular-nums">{Math.round(zoom * 100)}%</span>
        <button type="button" className="btn-ghost !px-2.5 !py-1" onClick={() => zoomBy(1.25)} disabled={zoom >= ZOOM_MAX} aria-label="Zoom in">+</button>
        <button type="button" className="btn-ghost !px-2.5 !py-1" onClick={() => setZoom(1)} disabled={zoom === 1}>Fit</button>
      </div>

      <div className="overflow-x-auto">
        <div style={{ minWidth: 340 + timelineMin }}>
          {/* Header */}
          <div
          ...
          >
            ...
            <div className="relative h-10" style={{ minWidth: timelineMin }}>
              {months.map((mo, i) => (
          ...
          {groups.map((g) => {
            ...
            const gLeft = gDated.length ? pct(Math.min(...gDated.map((i) => new Date(i.startDate!).getTime()))) : 0;
            const gRight = gDated.length ? pct(Math.max(...gDated.map((i) => new Date(i.endDate!).getTime()))) : 0;
            return (
              <div key={g.name}>
                {/* Group header row */}
                <div
                ...
                >
                  ...
                  <div />
                  <div />
                  <div className="relative h-7" style={{ minWidth: timelineMin }}>
                    {gDated.length > 0 && (
                      <div
                ...
                {g.items.map((it) => {
                  ...
                  const left = has ? pct(new Date(it.startDate!).getTime()) : 0;
                  const width = has ? Math.max(pct(new Date(it.endDate!).getTime()) - left, 0.6) : 0;
                  return (
                    <div
                      key={it.id}
                      ...
                      style={{ gridTemplateColumns: `${LEFT} 1fr` }}
                    >
                      <div className="truncate px-4 py-2 pl-6 text-stone-600">{it.taskName}</div>
